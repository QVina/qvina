...
//struct parallel_mc_task and typedef parallel_mc_task_container moved to parallel_mc.h
...
void merge_output_containers(const parallel_mc_task_container& many, output_container& out, fl min_rmsd, sz max_size) {
	...
	VINA_FOR_IN(i, many)
		merge_output_containers(((parallel_mc_task &)many[i]).out, out, min_rmsd, max_size);
	out.sort();
}
