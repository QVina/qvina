...
{ // not in the interface, used in assertions
	return eq(quaternion_norm_sqr(q), 1) && eq(boost::math::abs(q), 1);
}
bool eq(const qt &a, const qt &b)
{ // elementwise approximate equality - may return false for equivalent rotations
...
vec quaternion_to_angle(const qt &q)
{
	...
	if (c > -1 && c < 1)
	{								 // c may in theory be outside [-1, 1] even with approximately normalized q, due to rounding errors
...
{
	getV(quaternion_to_angle(q), out);
}
