...
void model::assign_types()
{
	VINA_FOR(i, grid_atoms.size() + atoms.size())
	{
		...
		case EL_TYPE_C:
			...
			break;
		case EL_TYPE_N:
			x = (acceptor && donor_NorO) ? XS_TYPE_N_DA : (acceptor ? XS_TYPE_N_A : (donor_NorO ? XS_TYPE_N_D : XS_TYPE_N_P));
			break;
		case EL_TYPE_O:
			x = (acceptor && donor_NorO) ? XS_TYPE_O_DA : (acceptor ? XS_TYPE_O_A : (donor_NorO ? XS_TYPE_O_D : XS_TYPE_O_P));
			break;
		case EL_TYPE_S:
			x = XS_TYPE_S_P;
			break;
...
fl model::eval(const precalculate &p, const igrid &ig, const vec &v, const conf &c)
{ // clean up
	set(c);
	fl e = evale(p, ig, v);
	VINA_FOR_IN(i, ligands)
...
fl model::eval_deriv(const precalculate &p, const igrid &ig, const vec &v, const conf &c, change &g)
{ // clean up
	set(c);
...
fl model::eval_intramolecular(const precalculate &p, const vec &v, const conf &c)
{
	...
	VINA_FOR_IN(i, other_pairs)
	{
		...
		if (find_ligand(pair.a) < ligands.size() || find_ligand(pair.b) < ligands.size())
