...
#include "../include/atom_constants.h"
#include "../include/file.h"
#include "../include/convert_substring.h"
...
struct parsing_struct
{
	...
	{ // no sub-branches besides immobile atom, including sub-sub-branches, etc
		VINA_FOR_IN(i, atoms)
		{
...
void postprocess_branch(non_rigid_parsed &nr, parsing_struct &p, context &c, B &b)
{
	...
	{ // postprocess children
		...
		VINA_FOR_IN(j, p_node.ps)
		{
			...
			if (!ps.essentially_empty())
			{																							 // immobile already inserted // FIXME ?!
...
{
	VINA_FOR_IN(i, p.atoms)
	{ // iterate over "root" of a "residue"
		parsing_struct::node &p_node = p.atoms[i];
	...
	{ // iterate over "root" of a "residue"
		...
		VINA_FOR_IN(j, p_node.ps)
		{
			parsing_struct &ps = p_node.ps[j];
			if (!ps.essentially_empty())
			{																									 // immobile atom already inserted // FIXME ?!
